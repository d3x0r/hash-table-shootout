...
static void expand_key_data_space( struct flower_hash_lookup_block *here ) {
	here->key_data_first_block = (uint8_t*)ReallocateAligned( here->key_data_first_block
...
struct flower_hash_lookup_block *InitFlowerHash( enum insertFlowerHashEntryOptions options ) {
	...
	expand_key_data_space( hash );
	if( options == IFHEO_DEFAULT) {
		//options = (enum insertFlowerHashEntryOptions)((int)IFHEO_CONVERTIBLE)|(int)IFHEO_DENSE;
		//options = IFHEO_CONVERTIBLE;
		options = IFHEO_IMMUTABLE;
	}
...
struct flower_hash_lookup_block* CreateFlowerHashMap( enum insertFlowerHashEntryOptions options, void* userDataBuffer, size_t userRecordSize, int maxRecords ) {
	...
	memset( hash, 0, sizeof( *hash ) );
	expand_key_data_space( hash );
	hash->entries = (struct key_data_entry*) userDataBuffer;
		...
		if( hash->info.dense || hash->info.immutable ) {
			if( !( next = hash->next_block[key[0] & HASH_MASK] ) ) {
				if( hash->info.convertible )
					...
					next = ( hash->next_block[key[0] & HASH_MASK] = InitFlowerHash( IFHEO_DEFAULT ) );
					next->parent = hash;
					next->info = hash->info;
					next->info.used = 0;
...
struct flower_hash_lookup_block* convertFlowerHashBlock( struct flower_hash_lookup_block* hash ) {
	...
	hash->next_block[imax]
		= ( new_dir_block
			= InitFlowerHash(IFHEO_DEFAULT) );
	new_dir_block->parent = hash;
...
struct flower_hash_lookup_block_cursor *flowerHashGetCursor( struct flower_hash_lookup_block *hash ){
	struct flower_hash_lookup_block_cursor *cursor = (struct flower_hash_lookup_block_cursor *)malloc( sizeof( *cursor));
	memset( cursor, 0, sizeof( *cursor ) );
	return cursor;
}


uintptr_t* flowerHashGet( struct flower_hash_lookup_block *hash, struct flower_hash_lookup_block_cursor*it ) {	
	int n;
	while( 1 ) {
		n = it->n;
		while(n < KEY_DATA_ENTRIES && !hash->key_data_offset[n]) {
			n++;
		}
		if( n < KEY_DATA_ENTRIES ){
			break;
		}

		int b;
		// check child blocks.
		for( b = 0; b <= HASH_MASK; b++ ) {
			if( hash->next_block[b] ) {
				hash = hash->next_block[b];
				it->levels[it->l++] = b+1; // check next...
				it->n = 0;
				break;
			}
		}
		// no more in this block... go up one..
		if( b == (HASH_MASK+1) ) {
			struct flower_hash_lookup_block *phash = hash->parent;
			do {
				it->l--;

				for( b = it->levels[it->l]; b <= HASH_MASK; b++ ){
					if( phash->next_block[b] ){
						hash = phash->next_block[b];
						it->levels[it->l++] = b+1; // again, resume next here...
						n = 0;
						break;
					}
				}
	...
	it->n = n+1; // check next coming back in.
	return &hash->entries[n %KEY_DATA_ENTRIES].stored_data;
}

void measureHash( struct flower_hash_lookup_block *hash, int *avail, int *used ) {
	int n;
	//printf( "Measuring hash %p (parent %p)  at %d %d", hash, hash->parent, avail[0], used[0] );
	for( n = 0; n < KEY_DATA_ENTRIES; n++ ){
		if( hash->key_data_offset[n] ) (*used)++;
		(*avail)++;
	}
	for( n = 0; n <= HASH_MASK; n++ ){
		if( hash->next_block[n]) {
			printf( "*");
		}else
			printf( " ");
	}
	...
	for( n = 0; n <= HASH_MASK; n++ ){
		if( hash->next_block[n]) {
			measureHash( hash->next_block[n], avail, used );
		}
	}
}

float getFlowerHashLoad( struct flower_hash_lookup_block *hash ) {
	int a, u;
	measureHash( hash, &a, &u );
	return(float)((double)u/(double)a);	
}
